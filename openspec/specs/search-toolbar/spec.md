...
The Filters surface SHALL provide a Clear action that resets search query and all filters to defaults (equivalent to the former Browse All reset), so the user can return to an unfiltered browse without a separate toolbar button.
...
- **AND** search results refresh according to those defaults
