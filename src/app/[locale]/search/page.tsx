...
const MATCH_VALUES = ["exact"] as const;

const providersParser = createParser({
  parse(query) {
    if (!query) return [];
...
const searchUrlParsers = {
  q: parseAsString.withDefault(""),
  status: parseAsString.withDefault("all"),
  genre: parseAsString.withDefault("all"),
  match: parseAsStringLiteral(MATCH_VALUES),
...
export default function SearchPage() {
  ...
  const [urlParams, setUrlParams] = useQueryStates(searchUrlParsers);
  ...
  const ingestProviderMeta = (data: {
    ...
    if (
      Array.isArray(data.availableProviders) &&
  ...
  const providerFilterLabel = () => {
    if (selectedProviders.length === 0) return t("allProviders");
          ...
          if (mode === "replace") {
            ...
            const providerCount =
              selected.length > 0
                ...
                : available.length > 0
                  ? available.length
                  : DEFAULT_PROVIDERS.length;
            captureEvent("search_performed", {
              query_length: query.trim().length,
              provider_count: providerCount,
            });
          } else if (pageNum > 1) {
            void setUrlParams({ page: pageNum });
          }
        } else if (mode === "replace") {
          setMangas([]);
          setSearchError(data.error ?? t("searchFailed"));
          setProviderNotices([]);
          setHasMore(false);
        }
      } catch (error) {
  ...
  // Initial ?q= deep link — one search on mount from nuqs URL state
  useEffect(() => {
    if (bootstrappedRef.current) return;
    bootstrappedRef.current = true;

    const { q, status, genre, match, providers } = urlParams;
    if (!q) return;

    setSearchQuery(q);
    setStatusFilter(status);
    setGenreFilter(genre);
    setExactMatch(match === "exact");
    ...
    filtersRef.current = {
      ...
      availableProviders: filtersRef.current.availableProviders,
    };

    void runSearch(q, 1, "replace");
    // urlParams read once on mount; bootstrappedRef prevents re-runs
          ...
          <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
            ...
            >
              <SheetHeader>
                ...
                <SheetDescription>{t("adjustFilters")}</SheetDescription>
              </SheetHeader>
              ...
              <div className="mt-4 space-y-4 pb-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="genre-filter">{t("genre")}</Label>
                  <Select
                    value={genreFilter}
                ...
                <div className="space-y-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-56">
                      <DropdownMenuLabel>{t("providers")}</DropdownMenuLabel>
                      <DropdownMenuSeparator />
        ...
        {(providerNotices.length > 0 || searchError) && (
          <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
            ...
            {providerNotices.length > 0 && (
              <p>
                Some providers unavailable: {providerNotices.join("; ")}.
