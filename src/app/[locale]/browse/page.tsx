...
const MODES = ["new", "latest", "trending"] as const;
const PERIODS = ["today", "week", "month"] as const;

const modeParser = parseAsStringLiteral(MODES).withDefault("new");
const periodParser = parseAsStringLiteral(PERIODS).withDefault("week");
...
export default function BrowsePage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery(browseQueryOptions(mode, period));

  const modeLabels: Record<BrowseMode, string> = {
    new: t("modeNew"),
    latest: t("modeLatest"),
    trending: t("modeTrending"),
  };

  const periodLabels: Record<BrowsePeriod, string> = {
    today: t("periodToday"),
    week: t("periodWeek"),
    month: t("periodMonth"),
  };
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {MODES.map((m) => (
            <Button
              ...
              className={
                mode === m ? "bg-blue-600 text-white hover:bg-blue-700" : ""
              }
            >
              {modeLabels[m]}
            </Button>
