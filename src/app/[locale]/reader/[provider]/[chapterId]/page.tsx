...
export default function ReaderPage({ params }: ReaderPageProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (mangaIdParam) {
        setManga(
          (prev) => prev ?? { id: mangaIdParam, title: "", provider: p },
        );
      }
      setLoading(false);
      captureEvent("chapter_opened", { provider: p });
  ...
  const handleReaderBack = () => {
    const fallback =
      manga?.id != null ? mangaPath(provider, manga.id) : "/dashboard";
  ...
  const onReaderPointerUp = () => {
    const wasTap = pointerStartRef.current !== null && !pointerMovedRef.current;
    ...
    return (
      <div className="flex min-h-dvh items-center justify-center bg-black">
        <div className="text-white text-center">
          ...
          <h1 className="text-2xl font-bold mb-4">{t("loadError")}</h1>
          <p className="text-gray-400 mb-6">
            {loadError || t("missingDescription")}
          </p>
  ...
  const { next: nextChapter, previous: previousChapter } = getChapterNeighbors(
    chapters,
    chapter.id,
  );
            ...
            <Popover open={settingsOpen} onOpenChange={setSettingsOpen}>
              ...
              >
                ...
                <div className="space-y-2">
                  <p className="text-xs text-gray-400">{t("orientation")}</p>
                ...
                <div className="space-y-2">
                  <p className="text-xs text-gray-400">{t("fit")}</p>
